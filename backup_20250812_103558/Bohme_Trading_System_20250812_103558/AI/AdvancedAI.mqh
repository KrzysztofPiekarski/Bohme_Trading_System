...
private:
    ...
    // Bias LSTM
    double m_bf[];      // Forget gate bias
    double m_bi[];      // Input gate bias
    double m_bc[];      // Candidate bias
    double m_bo[];      // Output gate bias
...
CLSTMNetwork::CLSTMNetwork(int input_size, int hidden_size, int output_size, int sequence_length) {
    ...
    ArrayResize(m_Wf, input_size);
    ArrayResize(m_Wi, input_size);
    ArrayResize(m_Wc, input_size);
    ArrayResize(m_Wo, input_size);
    
    for(int i = 0; i < input_size; i++) {
        ArrayResize(m_Wf[i], hidden_size);
        ArrayResize(m_Wi[i], hidden_size);
        ArrayResize(m_Wc[i], hidden_size);
        ArrayResize(m_Wo[i], hidden_size);
    }
    
    ArrayResize(m_bf, hidden_size);
    ArrayResize(m_bi, hidden_size);
    ArrayResize(m_bc, hidden_size);
    ArrayResize(m_bo, hidden_size);
    
    ArrayResize(m_hidden_state, hidden_size);
    ArrayResize(m_cell_state, hidden_size);
    
    // Inicjalizacja cache
    ArrayResize(m_hidden_cache, sequence_length);
    ArrayResize(m_cell_cache, sequence_length);
    ArrayResize(m_gate_cache, sequence_length);
    
    for(int i = 0; i < sequence_length; i++) {
        ArrayResize(m_hidden_cache[i], hidden_size);
        ArrayResize(m_cell_cache[i], hidden_size);
        ArrayResize(m_gate_cache[i], hidden_size * 4); // 4 gates
    }
...
bool CLSTMNetwork::Initialize() {
    // Inicjalizacja stanów i wag
...
bool CLSTMNetwork::InitializeWeights() {
    ...
    for(int i = 0; i < m_input_size; i++) {
        for(int j = 0; j < m_hidden_size; j++) {
            m_Wf[i][j] = (MathRand() % 1000) / 1000.0 * scale - scale/2;
            m_Wi[i][j] = (MathRand() % 1000) / 1000.0 * scale - scale/2;
            m_Wc[i][j] = (MathRand() % 1000) / 1000.0 * scale - scale/2;
            m_Wo[i][j] = (MathRand() % 1000) / 1000.0 * scale - scale/2;
        }
    }
...
double CLSTMNetwork::ForwardPass(double &sequence[]) {
    ...
    for(int t = 0; t < m_sequence_length; t++) {
        double input = sequence[t];
        
        // Gates
        double ft = 0.0, it = 0.0, ct_tilde = 0.0, ot = 0.0;
        
        for(int j = 0; j < m_hidden_size; j++) {
            // Forget gate
            double sum_f = m_bf[j];
            for(int i = 0; i < m_input_size; i++) {
                sum_f += m_Wf[i][j] * input;
            }
            sum_f += m_hidden_state[j] * 0.1; // Simplified recurrent connection
            ft = 1.0 / (1.0 + MathExp(-sum_f));
            
            // Input gate
            double sum_i = m_bi[j];
            for(int i = 0; i < m_input_size; i++) {
                sum_i += m_Wi[i][j] * input;
            }
            sum_i += m_hidden_state[j] * 0.1;
            it = 1.0 / (1.0 + MathExp(-sum_i));
            
            // Candidate
            double sum_c = m_bc[j];
            for(int i = 0; i < m_input_size; i++) {
                sum_c += m_Wc[i][j] * input;
            }
            sum_c += m_hidden_state[j] * 0.1;
            ct_tilde = MathTanh(sum_c);
            
            // Output gate
            double sum_o = m_bo[j];
            for(int i = 0; i < m_input_size; i++) {
                sum_o += m_Wo[i][j] * input;
            }
            sum_o += m_hidden_state[j] * 0.1;
            ot = 1.0 / (1.0 + MathExp(-sum_o));
            ...
            m_gate_cache[t][j*4 + 0] = ft;
            m_gate_cache[t][j*4 + 1] = it;
            m_gate_cache[t][j*4 + 2] = ct_tilde;
            m_gate_cache[t][j*4 + 3] = ot;
        }
        
        // Cache states
        ArrayCopy(m_hidden_cache[t], m_hidden_state);
        ArrayCopy(m_cell_cache[t], m_cell_state);
    }
    
    // Return output (simplified - average of hidden state)
    double output = 0.0;
    for(int j = 0; j < m_hidden_size; j++) {
        output += m_hidden_state[j];
    }
    return output / m_hidden_size;
}
...
bool CLSTMNetwork::BackwardPass(double &targets[], double learning_rate) {
    ...
    for(int t = m_sequence_length - 1; t >= 0; t--) {
        for(int j = 0; j < m_hidden_size; j++) {
            ...
            double gradient = (m_hidden_state[j] - targets[0]) * 0.1;
            
            // Update weights (simplified)
            for(int i = 0; i < m_input_size; i++) {
                m_Wf[i][j] -= learning_rate * gradient;
                m_Wi[i][j] -= learning_rate * gradient;
                m_Wc[i][j] -= learning_rate * gradient;
                m_Wo[i][j] -= learning_rate * gradient;
            }
            
            // Update bias
            m_bf[j] -= learning_rate * gradient;
            m_bi[j] -= learning_rate * gradient;
            m_bc[j] -= learning_rate * gradient;
            m_bo[j] -= learning_rate * gradient;
        }
    }
