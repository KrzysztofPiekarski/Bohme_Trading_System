...
public:
    ...
    // Trening
    bool Train(double &sequences[], int sequence_stride, double &targets[], int epochs);
...
private:
    ...
    // Wagi i bias - spłaszczone tablice 1D
    double m_filters[];       // Rozmiar: m_num_filters * m_filter_size * m_input_channels
    double m_bias[];
    
    // Gradienty
    double m_filter_gradients[]; // 1D
...
bool CLSTMNetwork::Train(double &sequences[], int sequence_stride, double &targets[], int epochs) {
    int num_sequences = ArraySize(targets);
    if(num_sequences <= 0 || sequence_stride != m_sequence_length) return false;
    ...
    for(int epoch = 0; epoch < epochs; epoch++) {
        ...
        for(int s = 0; s < num_sequences; s++) {
            double seq_local[];
            ArrayResize(seq_local, m_sequence_length);
            for(int t = 0; t < m_sequence_length; t++) {
                seq_local[t] = sequences[s * sequence_stride + t];
            }
            double prediction = ForwardPass(seq_local);
...
bool CConvolutionalNetwork::InitializeWeights() {
    // Losowa inicjalizacja filtrów i biasów w 1D
    int filter_area = m_filter_size * m_input_channels;
    int total_filter_params = m_num_filters * filter_area;
    ArrayResize(m_filters, total_filter_params);
    ArrayResize(m_filter_gradients, total_filter_params);
    ArrayResize(m_bias, m_num_filters);
    ArrayResize(m_bias_gradients, m_num_filters);
    for(int idx = 0; idx < total_filter_params; idx++) {
        m_filters[idx] = (MathRand() % 1000) / 1000.0 - 0.5;
        m_filter_gradients[idx] = 0.0;
    }
...
bool CTransformerNetwork::InitializeWeights() {
    // Prosta inicjalizacja wag do zera w 1D
    int attn_params = m_input_size * m_hidden_size;
    ArrayResize(m_Wq, attn_params); ArrayInitialize(m_Wq, 0.0);
    ArrayResize(m_Wk, attn_params); ArrayInitialize(m_Wk, 0.0);
    ArrayResize(m_Wv, attn_params); ArrayInitialize(m_Wv, 0.0);
    ArrayResize(m_Wo, attn_params); ArrayInitialize(m_Wo, 0.0);
    int ff_params = m_hidden_size * m_hidden_size;
    ArrayResize(m_W1, ff_params); ArrayInitialize(m_W1, 0.0);
    ArrayResize(m_W2, ff_params); ArrayInitialize(m_W2, 0.0);
    ArrayResize(m_b1, m_hidden_size); ArrayInitialize(m_b1, 0.0);
    ArrayResize(m_b2, m_hidden_size); ArrayInitialize(m_b2, 0.0);
    ArrayResize(m_attention_cache, 0);
    ArrayResize(m_ff_cache, 0);
    ArrayResize(m_gradient_cache, 0);
    m_is_trained = false;
    return true;
}
